<template>
  <div>
    <label class="block font-normal text-sm text-gray-100 mb-0.5">Страховая сумма</label>
    <p class="mb-2.5">
      $ {{ $thousandSeparator(value * 1000) }}
    </p>
    <Slider v-model="value" :step="1"/>
  </div>
</template>

<script setup>
import Slider from '@vueform/slider'
const { $thousandSeparator } = useNuxtApp()

const value = ref(30)

const valueInHundred = computed(() => {
  return value * 100
})
...
.slider-horizontal .slider-tooltip-top:before{
  ...
  bottom:calc(var(--slider-tooltip-arrow-size, 5px)*-2);content:"";
  height:0;
  left:50%;
  position:absolute;
  transform:translate(-50%);
...
.slider-horizontal .slider-tooltip-bottom:before{
  border:var(--slider-tooltip-arrow-size,5px) solid transparent;border-bottom-color:inherit;
  content:"";
  height:0;
  left:50%;
  position:absolute;
  top:calc(var(--slider-tooltip-arrow-size, 5px)*-2);
  transform:translate(-50%);width:0
}
