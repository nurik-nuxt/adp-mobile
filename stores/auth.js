...
export const useAuthStore = defineStore('auth', {
    ...
    actions: {
        ...
        async sendVerifyCode({ mobile_phone }) {
            const { apiBaseUrl, codeSms } = useRuntimeConfig().public
            let body = {
                t: codeSms,
                mobile_phone: mobile_phone
            }
            try {
                let resp = await $fetch(`${apiBaseUrl}/notification/verify/send-sms-code`, {
                    method: 'POST',
                    body,
                    headers: {
                        Accept: 'application/json',
        ...
        async registration({ body }) {
            const { authBaseUrl } = useRuntimeConfig().public
            try {
                let resp = await $fetch(`${authBaseUrl}/auth/temp-users/register`, {
                    method: 'POST',
                    body,
                    headers: {
                        Accept: 'application/json',
